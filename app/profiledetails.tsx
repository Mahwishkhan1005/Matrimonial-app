...
const ProfileDetails = () => {
    ...
    const fetchDetails = async () => {
      try {
        if (profileId) {
          ...
          const response = await api.get(`/user/profile/${String(profileId).trim()}`);
          ...
          const [profileRes, contactRes, familyRes, imagesRes] = await Promise.allSettled([
          ...
          if (contactRes.status === 'fulfilled') setContactDetails(contactRes.value.data);
          if (familyRes.status === 'fulfilled') setFamilyDetails(familyRes.value.data);
          if (imagesRes.status === 'fulfilled') setUserImages(imagesRes.value.data || []);
        }
      } catch (error) {
  ...
  const galleryImages = useMemo(() => {
    ...
    if (userImages && userImages.length > 0) {
      images = userImages.map(img => typeof img === 'string' ? { uri: img } : { uri: img.imageUrl || img.url });
    } 
    // 2. Fallback to images array in profileData
    else if (profileData?.images && profileData.images.length > 0) {
      images = profileData.images.map((img: any) => typeof img === 'string' ? { uri: img } : { uri: img.imageUrl || img.url });
    }
    ...
    <View className="flex-1 bg-[#F0F7FA]">
      ...
      <ScrollView className="flex-1 p-6" showsVerticalScrollIndicator={false}>
          ...
          <View className="w-20">
            <ScrollView showsVerticalScrollIndicator={false} style={{ maxHeight: 320 }}>
              {galleryImages.map((img, idx) => {
                const isSelected = (typeof mainImage === 'number' && mainImage === img) || 
                                 (typeof mainImage === 'object' && typeof img === 'object' && mainImage?.uri === img?.uri);
                return (
                  <TouchableOpacity
                    key={idx}
        ...
        <AccordionItem
          ...
          content={
            <View>
              <DetailRow
                label="Name"
                value={`${profileData?.name || ""} ${profileData?.surname || ""}`.trim() || (isMale ? "Rahul Verma" : "Bhavya Sharma")}
              />
              <DetailRow label="Profile ID" value={profileData?.profileId || "N/A"} />
              <DetailRow label="Age" value={calculateAge(profileData?.birthYear)} />
              <DetailRow label="Height" value={profileData?.height || "N/A"} />
              <DetailRow label="Weight" value={profileData?.weight || "N/A"} />
              <DetailRow label="Complexion" value={profileData?.complexion || "N/A"} />
              <DetailRow label="Marital Status" value={profileData?.maritalStatus || "N/A"} />
              <DetailRow label="Education" value={profileData?.education || "N/A"} />
              <DetailRow label="Profession" value={profileData?.profession || "N/A"} />
              <DetailRow label="Annual Income" value={profileData?.annualIncome || "N/A"} />
            </View>
          }
        />
        ...
        <AccordionItem
          ...
          content={
            <View>
              <DetailRow label="Birth Place" value={profileData?.birthPlace || "N/A"} />
              <DetailRow label="Birth Time" value={`${profileData?.birthHour || ""}:${profileData?.birthMinute || ""} ${profileData?.birthPeriod || ""}`} />
              <DetailRow label="Gothram" value={profileData?.gothram || "N/A"} />
        ...
        <AccordionItem
          ...
          content={
            <View>
              <DetailRow label="Father Name" value={familyDetails?.fatherName || "N/A"} />
              <DetailRow label="Father Occupation" value={familyDetails?.fatherOccupation || "N/A"} />
              <DetailRow label="Mother Name" value={familyDetails?.motherName || "N/A"} />
              <DetailRow label="Mother Occupation" value={familyDetails?.motherOccupation || "N/A"} />
              <DetailRow label="Family Type" value={familyDetails?.familyType || "N/A"} />
              <DetailRow label="Status" value={familyDetails?.familyStatus || "N/A"} />
              <DetailRow label="Values" value={familyDetails?.familyValues || "N/A"} />
              <DetailRow label="Brothers" value={`${familyDetails?.brothersCount || 0} (${familyDetails?.brothersMarried || 0} Married)`} />
              <DetailRow label="Sisters" value={`${familyDetails?.sistersCount || 0} (${familyDetails?.sistersMarried || 0} Married)`} />
              <DetailRow label="Family Location" value={familyDetails?.familyLocation || "N/A"} />
              <DetailRow label="Native Place" value={familyDetails?.nativePlace || "N/A"} />
              <DetailRow label="Income" value={familyDetails?.familyIncome || "N/A"} />
              <DetailRow label="Own House" value={familyDetails?.ownHouse ? "Yes" : "No"} />
              <DetailRow label="Religion" value={familyDetails?.religion || "N/A"} />
              <DetailRow label="Caste" value={familyDetails?.caste || "N/A"} />
              {familyDetails?.subCaste ? <DetailRow label="Sub Caste" value={familyDetails.subCaste} /> : null}
              <View className="mt-2">
                <Text className="text-gray-500 font-medium text-sm">About Family</Text>
                <Text className="text-[#2D89B5] text-sm mt-1 leading-5">{familyDetails?.aboutFamily || "N/A"}</Text>
              </View>
            </View>
          }
        />
        ...
        <AccordionItem
          ...
          content={
            <View>
              <DetailRow label="Mobile" value={contactDetails?.phoneNumber || profileData?.phoneNumber || "N/A"} />
              {contactDetails?.alternatePhoneNumber ? (
                <DetailRow label="Alt Mobile" value={contactDetails.alternatePhoneNumber} />
              ) : null}
              <DetailRow label="Email" value={contactDetails?.email || profileData?.email || "N/A"} />
              <DetailRow 
                label="Location" 
                value={`${contactDetails?.address || ""}, ${contactDetails?.city || ""}, ${contactDetails?.state || ""}, ${contactDetails?.country || ""} - ${contactDetails?.pincode || ""}`.replace(/^, /, "").replace(/ ,/g, "").replace(/, ,/g, ",").trim().replace(/,$/, "") || "N/A"} 
              />
            </View>
          }
        />

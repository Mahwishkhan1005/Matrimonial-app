...
const EditProfile = () => {
        ...
        try {
          ...
          if (contactRes.status === "fulfilled" && contactRes.value.data) {
            ...
            newFormData = {
              ...
              city: cData.city || "Select",
              state: cData.state || "Select",
              country: cData.country || "India",
              pincode: cData.pincode || "",
              // Use email from contact if profile didn't have it
              email: newFormData.email || cData.email || "",
            };

            // Fetch districts for initial state if exists
            if (cData.state && cData.state !== "Select") {
              try {
                const distRes = await api.get(`/contact/districts/${cData.state}`);
                setDistricts(distRes.data || []);
              } catch (err) {
                console.error("Error fetching initial districts:", err);
              }
            }
          }
  ...
  const handleUpdateProfile = async () => {
    ...
    for (const field of mandatoryEnumFields) {
      if (!formData[field.key as keyof typeof formData] || formData[field.key as keyof typeof formData] === "Select") {
        Alert.alert("Selection Required", `Please select a value for ${field.label}`);
  ...
  const updateField = async (field: string, value: string) => {
    ...
    if (field === "state") {
      if (value && value !== "Select") {
        try {
          const response = await api.get(`/contact/districts/${value}`);
          setDistricts(response.data || []);
          // Reset city when state changes
          setFormData((prev) => ({ ...prev, city: "Select" }));
        } catch (error) {
